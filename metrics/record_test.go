...
func TestRecord(t *testing.T) {
	...
	t.Run("TestRecordValueWithDifferentPolicies", func(t *testing.T) {
		...
		if avgRecord.Value() != 30 { // 90 / 3 = 30
			t.Errorf("Expected Value() with PolicyAvg to return 30, got %v", avgRecord.Value())
		}

		// 测试PolicySum (应该直接返回value)
		sumRecord := Record{
			metrics: counter, // counter has PolicySum
			value:   50,
			cnt:     2,
		}
		if sumRecord.Value() != 50 {
			t.Errorf("Expected Value() with PolicySum to return 50, got %v", sumRecord.Value())
		}
		...
		if stopwatchRecord.Value() != 250 { // 1000 / 4 = 250
			t.Errorf("Expected Value() with PolicyStopwatch to return 250, got %v", stopwatchRecord.Value())
		}
...
func TestRecordMerge(t *testing.T) {
	...
	// 创建不同策略的Metrics
	counter := getCounter("test_counter", "test_group") // PolicySum
	gauge := getGauge("test_gauge", "test_group")       // PolicySet
	maxGauge := getMaxGauge("test_max", "test_group")   // PolicyMax
	minGauge := getMinGauge("test_min", "test_group")   // PolicyMin
	avgGauge := getAvgGauge("test_avg", "test_group")   // PolicyAvg
	...
	tests = append(tests, struct {
		...
		expected    Value
		expectError bool
	}{"PolicySum Merge", PolicySum, Record{
		metrics:    counter,
		value:      10,
		cnt:        1,
		dimensions: dimensions,
	...
	tests = append(tests, struct {
		...
		expected    Value
		expectError bool
	}{"PolicySet Merge", PolicySet, Record{
		metrics:    gauge,
		value:      10,
		cnt:        1,
		dimensions: dimensions,
	...
	tests = append(tests, struct {
		...
		expected    Value
		expectError bool
	}{"PolicyMax Merge", PolicyMax, Record{
		metrics:    maxGauge,
		value:      15,
		cnt:        1,
		dimensions: dimensions,
	...
	tests = append(tests, struct {
		...
		expected    Value
		expectError bool
	}{"PolicyMin Merge", PolicyMin, Record{
		metrics:    minGauge,
		value:      30,
		cnt:        1,
		dimensions: dimensions,
	...
	tests = append(tests, struct {
		...
		expected    Value
		expectError bool
	}{"PolicyAvg Merge", PolicyAvg, Record{
		metrics:    avgGauge,
		value:      40,
		cnt:        2,
		dimensions: dimensions,
	...
	tests = append(tests, struct {
		...
		expected    Value
		expectError bool
	}{"Different Names", PolicySum, Record{
	...
	tests = append(tests, struct {
		...
		expected    Value
		expectError bool
	}{"Different Groups", PolicySum, Record{
	...
	tests = append(tests, struct {
		...
		expected    Value
		expectError bool
	}{"Different Policies", PolicySum, Record{
		metrics:    counter,
		value:      10,
		cnt:        1,
		dimensions: dimensions,
	...
	tests = append(tests, struct {
		...
		expected    Value
		expectError bool
	}{"Different Dimensions", PolicySum, Record{
		metrics:    counter,
		value:      10,
		cnt:        1,
		dimensions: Dimension{"host": "server1"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				if tt.policy == PolicyAvg || tt.policy == PolicyStopwatch {
