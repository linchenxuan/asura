...
func InitTracing() (Tracer, error) {
	...
	// Create and load configuration with fallback mechanism
	...
	// Register configuration change listener for hot-reload capability
	configMgr.AddChangeListener(&tracingConfigListener{})

	return tracer, nil
}

// Configuration change listener for hot-reloading tracing configuration
// Implements the config.ChangeListener interface to handle runtime configuration updates
type tracingConfigListener struct{}

// OnConfigChanged handles configuration changes for the tracing system
// This enables hot-reload capability without restarting the application
// Parameters:
//   - configName: Name of the configuration that changed
//   - newConfig: The new configuration values
//   - oldConfig: The previous configuration values
//
// Returns:
//   - error: Any error encountered during configuration update
//
// Thread safety: This method is called from configuration manager's goroutine
func (l *tracingConfigListener) OnConfigChanged(configName string, newConfig, oldConfig config.Config) error {
	...
	// Build new tracer with updated configuration
	tracer, err := buildTracer(newCfg)
	if err != nil {
		return err
	}

	// Update global tracer atomically
	setGlobalTracer(tracer)

	// Re-register propagators to ensure they're available in the new tracer
	registerDefaultPropagators()
