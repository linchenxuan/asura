...
func TestMetricsIntegration(t *testing.T) {
	...
	t.Run("TestMixedMetricsUsage", func(t *testing.T) {
		...
		for _, record := range records {
			if record.Metrics().Name() == "request_count" && record.Metrics().Policy() == PolicySum {
				foundCounter = true
			} else if record.Metrics().Name() == "memory_usage" && record.Metrics().Policy() == PolicySet {
				foundGauge = true
			} else if record.Metrics().Name() == "operation_time" && record.Metrics().Policy() == PolicyStopwatch {
				foundStopwatch = true
			}
		}
